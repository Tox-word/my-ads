...
async def use_promo_cmd(message: types.Message):
    ...
    promo = db.get_promo(code_text) # [code, reward, uses_left, chan_id]
    
    if not promo or promo[2] <= 0:
        return await message.answer("❌ Промокод не существует или закончился.")
    
    # ПРОВЕРКА ПОДПИСКИ ДЛЯ ПРОМО
    try:
        chat_member = await bot.get_chat_member(promo[3], message.from_user.id)
        if chat_member.status not in ["member", "administrator", "creator"]:
            return await message.answer(f"❌ Чтобы активировать, подпишись на канал: {promo[3]}")
    except:
        return await message.answer("⚠️ Ошибка проверки подписки на канал промокода.")
